...
async function generateCellPerformance(supabase: any, churchId: string, filters: any) {
  ...
  const leaderStats = cellsPerformance?.reduce((acc: Record<string, any>, cell: any) => {
...
async function generateLeadershipDevelopment(supabase: any, churchId: string, filters: any) {
  ...
  const roleDistribution = leadershipStats?.reduce((acc: Record<string, number>, profile: any) => {
  ...
  data.timoteo_development = {
    ...
    avg_success_score: timoteoStats?.reduce((sum: number, member: any) => 
      sum + (member.success_ladder_score || 0), 0
    ) / (timoteoStats?.length || 1),
    timoteos_by_cell: timoteoStats?.reduce((acc: Record<string, number>, member: any) => {
